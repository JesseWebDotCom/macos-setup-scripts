...
__status_changed() {
  ...
  percentage=$(math_calc "$(math_calc "$StepsDone/$TotalSteps")*100.00")

  ((__int_percentage=$(math_round "$percentage")))
...
__progress_string() {
  ...
  BarDone=$(math_max 0 "$(math_min $BarSize "$(math_floor "$(math_calc "$Percent*$BarSize")")")")
...
__draw_status_line(){
  ...
  current_percent=$(math_calc "$percentage/100.00")
...
bar_stop() {
  ...
  if (( ! ${E_START_INVOKED:-0} )); then
    echo "Warn: bar_stop called but bar_start was not invoked" >&2 
    echo "Returning.." # Exit or return?
    return 1
  fi
  #-- Reset bar_start check
  E_START_INVOKED=0
...
bar_status_changed() {
  if (( ! ${E_START_INVOKED:-0} )); then
    echo "ERR: bar_start not called" >&2
    echo "Exiting.."
    exit 1
  fi
